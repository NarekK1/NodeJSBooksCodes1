...
const readsFile = async (filepath) => {
    ...
    original = await Promise.all(
        files.map(async (file) => {
            const frame = await fs.readFile(path.join(filepath, file), 'utf8');
...
const server = http.createServer((req, res) => {
    ...
    if (!original.length) {
        stream.push('No frames loaded. Check the frames directory path.\n');
        stream.push(null);
        return;
    }
...
await readsFile(fileDir).catch(err => {
